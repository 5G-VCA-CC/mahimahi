...
CFG="${1:-exp_40ms_50mbps_dual.yaml}"
...
# NEW: global UDP flow rate (as in your YAML: UDP_FLOW_RATE: 50)
UDP_FLOW_RATE="$(yaml_get '.UDP_FLOW_RATE')"
: "${UDP_FLOW_RATE:=50}"
...
run_one() {
  ...
  (
    ...
    bash -lc '
      ...
      : "${UDP_FLOW_RATE:?missing UDP_FLOW_RATE}"

      cli_pids=()

      # Classic = UDP blast at UDP_FLOW_RATE (in Mbit/s)
      if [[ "$CLASSIC_ENABLED" == "1" ]]; then
        (
