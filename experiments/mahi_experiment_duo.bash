...
BASE_PORT=5300
PORT_L4S=$((BASE_PORT + 2*IDX))
PORT_CLASSIC=$((PORT_L4S + 1))
...
CLASSIC_CLIENT_CORE="${CLASSIC_CLIENT_CORE:-4}"

OUT_FILE="${OUT_DIR}/output_duo_${IDX}.txt"
FLOW_L4S="${OUT_DIR}/iperf_l4s_${IDX}.txt"
FLOW_CLASSIC="${OUT_DIR}/iperf_classic_${IDX}.txt"

mkdir -p "$OUT_DIR"

echo "[*] Cleaning up Mahimahi + iperf..."
# Cleanup needs sudo because netns are root-owned
...
echo "[*] Running Mahimahi dual flow (IDX=$IDX)..."
...
mm-delay 0 mm-link --meter-all \
  ...
  "$TRACE_UP" "$TRACE_DOWN" -- bash -c "
    ...
    echo '[+] Classic flow: port=$PORT_CLASSIC tos=$CLASSIC_TOS'
