#!/usr/bin/env bash
set -euo pipefail

# File: SHIELD_SINGLE_l4S.bash
# CPU shielding wrapper for the single L4S-marked flow experiment.
#
# Usage:
#   sudo ./SHIELD_SINGLE_l4S.bash <seconds_per_run> <num_runs> [start_index]
#
# Example:
#   sudo ./SHIELD_SINGLE_l4S.bash 20 10 0

SECS=${1:-20}
NUM_RUNS=${2:-1}
START_INDEX=${3:-0}

RUN_USER="${SUDO_USER:-$(id -un)}"

HOUSEKEEPING="0-3"
EXPERIMENT_CPUS="4-7"

MAHI_CORE=7
SERVER_CORE=5
L4S_CLIENT_CORE=6

echo "[*] Resetting cset..."
sudo cset shield --reset || true

echo "[*] Shielding CPUs: $EXPERIMENT_CPUS (housekeeping: $HOUSEKEEPING)"
sudo cset shield --cpu="$EXPERIMENT_CPUS" --kthread=on

# Optional: ask for sudo once up front (still may expire if runs are long)
sudo -v

for r in $(seq 0 $((NUM_RUNS - 1))); do
  idx=$((START_INDEX + r))
  echo ""
  echo "[*] ===== RUN index=$idx ====="

  # Execute inside the shield, but drop privileges to RUN_USER so Mahimahi is non-root
  sudo cset shield --exec -- sudo -u "$RUN_USER" --preserve-env=PATH,HOME,USER bash -lc "
    export SERVER_CORE=$SERVER_CORE
    export L4S_CLIENT_CORE=$L4S_CLIENT_CORE
    taskset -c $MAHI_CORE ./mahi_experiment_l4s_single.bash $SECS $idx
  "

  sleep 3
done

echo "[*] Releasing cset..."
sudo cset shield --reset
