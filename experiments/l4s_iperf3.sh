...
# Usage:
#   ./run_mahi_cset_single_l4s.bash <seconds_per_run> <num_runs> [mahi_core] [start_index] [server_core] [client_core]
#
# Examples:
#   ./run_mahi_cset_single_l4s.bash 20 10 7          # 10 runs, 20s each, mahi core 7, start 0, server 5, client 6 (defaults)
#   ./run_mahi_cset_single_l4s.bash 25 50 7 100      # 50 runs, start at output_cset100..149 (defaults server=5 client=6)
#   ./run_mahi_cset_single_l4s.bash 15 5             # 5 runs, 15s each, mahi core 0, server 5, client 6 (defaults)
#   ./run_mahi_cset_single_l4s.bash 20 10 7 0 5 6    # explicit: mahi=7, server=5, client=6
...
for r in $(seq 0 $((NUM_RUNS - 1))); do
  ...
  # We switch the CC and ECN INSIDE the mm namespace using ip netns exec.
  as_user bash -lc "
    taskset -c '${MAHI_CORE}' mm-delay '${DELAY}' mm-link --meter-all \
      ...
      '${TRACE_UP}' '${TRACE_DOWN}' -- bash -lc '
        ...
        echo \"[+] cset index=${idx} starting L4S TCP flow -> port ${port}\"

        # Find mahimahi sender namespace (mm-*)
        SENDER_NS=\$(ip netns list | awk \"{print \\$1}\" | grep -E \"^mm-\" | head -n1 || true)
        if [[ -z \"\$SENDER_NS\" ]]; then
          echo \"[!] Could not find mm-* netns. Aborting run.\"
          exit 1
        fi

        # Enable ECN and set Prague congestion control inside the sender namespace.
        # NOTE: sysctl requires root, so use sudo + ip netns exec.
        sudo ip netns exec \"\$SENDER_NS\" sysctl -w net.ipv4.tcp_ecn=1 >/dev/null || true

        # Try to switch CC to prague if available
        if sudo ip netns exec \"\$SENDER_NS\" sysctl net.ipv4.tcp_available_congestion_control 2>/dev/null | grep -qw prague; then
          sudo ip netns exec \"\$SENDER_NS\" sysctl -w net.ipv4.tcp_congestion_control=prague >/dev/null || true
        fi

        echo \"[+] netns=\$SENDER_NS ECN=\$(sudo ip netns exec \"\$SENDER_NS\" sysctl -n net.ipv4.tcp_ecn 2>/dev/null || echo ?)\"
        echo \"[+] CC=\$(sudo ip netns exec \"\$SENDER_NS\" sysctl -n net.ipv4.tcp_congestion_control 2>/dev/null || echo ?)\"

        # Run TCP iperf3 (no -u). This is the L4S-ish flow (ECN + prague CC + DualPI2).
        taskset -c ${CLIENT_CORE} iperf3 -c 10.0.0.1 -p ${port} -t ${SECS} --interval 1 \
          2>&1 | tee \"${FLOW_FILE}\"

        echo \"[+] cset index=${idx} finished L4S flow\"
      ' >> '${OUT_FILE}' 2>&1
  " || true
