...
run_one() {
  ...
  # Note: We already cleared os processes OUT of these cores via cset shield
  ...
  # IMPORTANT: run mm-delay/mm-link INSIDE the cset shield user cpuset
  sudo -u "$RUN_USER" \
    taskset -c "$MAHIMAHI_CORE" \
      mm-delay 0 mm-link \
        ...
        bash -lc "
          ...
          (
            taskset -c "$CLIENT_CORE_L4S" iperf3 -c 10.0.0.1 -p $port2 -u \
              -b $CLASSIC_RATE -l $CLASSIC_PACKET_LEN \
              -t $SECS --tos $CLASSIC_TOS
          ) &
          wait
...
# enter shield ONCE with these exports and run everything inside
