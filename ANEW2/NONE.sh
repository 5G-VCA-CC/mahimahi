...
# Optional knobs (empty = do nothing)
L4S_TOS="$(yaml_get '.flows.l4s.tos')"
L4S_CC="$(yaml_get '.flows.l4s.cc')"                       # empty => don't set CC anywhere
FORCE_ECN="$(yaml_get '.flows.l4s.force_ecn')"             # "true"/"false"/empty
BASE_PORT="$(yaml_get '.flows.base_port')"

# Optional host baseline (empty => don't touch host settings)
HOST_CC_BASELINE="$(yaml_get '.host.baseline_cc')"         # e.g., "cubic" / "reno" / empty
HOST_ECN_BASELINE="$(yaml_get '.host.baseline_ecn')"       # e.g., "0"/"1"/"2" / empty

: "${SECS:=30}"
: "${NUM_RUNS:=1}"
: "${BASE_PORT:=5300}"
: "${L4S_CC:=}"              # IMPORTANT: do NOT default to prague
: "${FORCE_ECN:=}"           # empty => don't force
: "${HOST_CC_BASELINE:=}"    # empty => don't touch host CC
: "${HOST_ECN_BASELINE:=}"   # empty => don't touch host ECN
...
# --- host cc/ecn capture + optional baseline + restore ---
HOST_CC_ORIG="$(sysctl -n net.ipv4.tcp_congestion_control 2>/dev/null || true)"
HOST_ECN_ORIG="$(sysctl -n net.ipv4.tcp_ecn 2>/dev/null || true)"

restore_host() {
  # restore only if we successfully read originals
  if [[ -n "${HOST_CC_ORIG:-}" ]]; then
    sysctl -w net.ipv4.tcp_congestion_control="$HOST_CC_ORIG" >/dev/null 2>&1 || true
  fi
  if [[ -n "${HOST_ECN_ORIG:-}" ]]; then
    sysctl -w net.ipv4.tcp_ecn="$HOST_ECN_ORIG" >/dev/null 2>&1 || true
  fi
}

apply_host_baseline_if_requested() {
  if [[ -n "$HOST_CC_BASELINE" ]]; then
    sysctl -w net.ipv4.tcp_congestion_control="$HOST_CC_BASELINE" >/dev/null
  fi
  if [[ -n "$HOST_ECN_BASELINE" ]]; then
    sysctl -w net.ipv4.tcp_ecn="$HOST_ECN_BASELINE" >/dev/null
  fi
}

trap 'restore_host; cset shield --reset >/dev/null 2>&1 || true; cleanup_all' EXIT

# Helpful warning: if you didn't set cc, you'll inherit host cc (maybe prague)
if [[ -z "$L4S_CC" ]]; then
  cur_cc="$(sysctl -n net.ipv4.tcp_congestion_control 2>/dev/null || true)"
  if [[ "$cur_cc" == "prague" ]]; then
    echo "[!] WARNING: flows.l4s.cc is empty, so Mahimahi will inherit host default CC."
    echo "    Host tcp_congestion_control is currently 'prague' => your run will still use prague."
    echo "    Fix: either set host.baseline_cc (e.g., cubic) OR set flows.l4s.cc explicitly."
  fi
fi
...
run_one() {
  ...
  # Apply requested host baseline BEFORE creating mm netns (so new mm netns inherits it)
  apply_host_baseline_if_requested
  ...
  # Run Mahimahi under shield + pinned core
  sudo -u "$RUN_USER" \
  taskset -c "$MAHIMAHI_CORE" \
    mm-delay 0 mm-link \
      ...
      bash -lc "
        ...
        # Optional: set CC inside mm namespace only if flows.l4s.cc is provided
        if [[ -n \"${L4S_CC:-}\" ]]; then
          if ! sysctl -n net.ipv4.tcp_available_congestion_control | tr ' ' '\n' | grep -qx \"${L4S_CC}\"; then
            echo \"[!] Congestion control '${L4S_CC}' not available inside Mahimahi namespace.\"
            echo \"    Available: \$(sysctl -n net.ipv4.tcp_available_congestion_control)\"
            exit 2
          fi
          sysctl -w net.ipv4.tcp_congestion_control=\"${L4S_CC}\" >/dev/null
        fi

        # Optional: force ECN inside mm namespace only if flows.l4s.force_ecn is 'true'
        if [[ \"${FORCE_ECN:-}\" == \"true\" ]]; then
        ...
        (
          ...
          taskset -c \"$CLIENT_CORE_CLASSIC\" iperf3 -c 10.0.0.1 -p $port1 -u \
            ...
            -t \"$SECS\" ${CLASSIC_TOS:+--tos \"$CLASSIC_TOS\"}
        ) &

        (
          # l4s flow: TCP (CC/TOS only if configured)
          cmd=(taskset -c \"$CLIENT_CORE_L4S\" iperf3 -c 10.0.0.1 -p $port2 -t \"$SECS\")
          if [[ -n \"${L4S_CC:-}\" ]]; then
            cmd+=(-C \"${L4S_CC}\")
          fi
          if [[ -n \"${L4S_TOS:-}\" ]]; then
...
export L4S_TOS L4S_CC FORCE_ECN
export HOST_CC_BASELINE HOST_ECN_BASELINE
export RUN_USER

export -f next_index
export -f cleanup_all
export -f run_one
export -f apply_host_baseline_if_requested
